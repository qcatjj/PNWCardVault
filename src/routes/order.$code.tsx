  ...
  return (
    <main className="mx-auto max-w-xl px-4 py-16 sm:px-6">
      ...
      <p className="mt-3 text-sm text-muted-foreground">
        {paid
          ? `Charged ${brandLabel(stripe.brand)} •••• ${stripe.last4} through Stripe${stripe.mode === "test" ? " in this preview" : ""}. Stock is down. Keep the order code — that is the packing slip.`
          : "Sold from the desk. Stock is down on those listings. Share the receipt link if you need to pull it back up."}
      </p>
      <div className="mt-8 rounded-xl border border-border bg-card p-6">
        <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Order code</p>
      ...
      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <ShareButton path={`/order/${order.code}`} title={`PNW Card Hub ${order.code}`} label="Copy receipt link" />
        <Link
          to="/shop"
